...
$(document).ready(function () { 
    $("#layout1").ligerLayout({ 
        ...
        topHeight: 40 });
    $("input[name='rdo_Date']").click(
        function(){  
            var rdoValue=$("input[name='rdo_Date']:checked").val();
            if (rdoValue=='month') {
                $("#lbl_Month").hide();
                $("#lbl_Month").val(0);
            }else if(rdoValue=='day'){
                $("#lbl_Month").show();
            }
        }
    );    
    ...
    grid =  $("#maingrid4").createLigerGrid({
        header: [
            {display:'时间',name:'DateStr'},
            { display: '数量', name: 'LabelNum' }
        ],
        leftBtns:{show:false},
        ID: "maingrid4",
        ajaxURL: "/Chart/getLabelChartByDate",
        param: { 'AreaID': AreaID, 'CoFarmerIDs': $("#hd_CoFarmerId").val(), 'GoodsID': $("#hd_GoodsId").val(), "Year": $("#txt_Year").val(),"Month":$("#txt_Month").val() },
        onLoaded: function () {
            DrawChart(type);
        }
    });                        
        ...
        if (grid.data.data.length > 0) {
            ...
            for (var p in jdatas) {
                if (jdatas[p] && jdatas[p] != undefined) {
                    Ydatas.push({ name: jdatas[p]["DateStr"], y: Number(jdatas[p]["LabelNum"]) });
                    LabelCount+=Number(jdatas[p]["LabelNum"]) 
                    xlabels.push(jdatas[p]["DateStr"]);
                }
            }                       
        }
...
function Query() {
    ...
    grid.setParm("Year", $("#txt_Year").val());
    grid.setParm("Month", $("#txt_Month").val());
    grid.reload();
}
function openParam()
{
