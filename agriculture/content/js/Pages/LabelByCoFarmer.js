...
$(document).ready(function () { 
    ...
    grid =  $("#maingrid4").createLigerGrid({
        header: [
            {display:'时间',name:'CoFarmer_Name'},
            { display: '数量', name: 'LabelNum' }
        ],
        leftBtns:{show:false},
        ID: "maingrid4",
        ajaxURL: "/Chart/getLabelChartByCoFarmer",
        ...
        if (grid.data.data.length > 0) {
            ...
            for (var p in jdatas) {
                if (jdatas[p] && jdatas[p] != undefined) {
                    Ydatas.push({ name: jdatas[p]["CoFarmer_Name"], y: Number(jdatas[p]["LabelNum"]) });
                    LabelCount+=Number(jdatas[p]["LabelNum"]) 
                    xlabels.push(jdatas[p]["CoFarmer_Name"]);
                }
            }                       
        }
