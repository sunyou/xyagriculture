...
<head>
    ...
    <script type="text/javascript">  
        ...
        var headers = [
                    { display: '农作作物名称', name: 'Goods_Name' ,width:'150' },
                    { display: '农作作物编号', name: 'GoodsSTD_No',width:'100' },
                    { display: '农作作物分类', name: 'GoodsType_Name',width:'200' }];
        $(document).ready(function () {
            multiCheck = GetQueryString("__CheckBox");
            if (multiCheck == undefined) {
                multiCheck = false;
                headers.unshift({ display: '选择',width:'60', render: function (record, rowindex, value, column) { return "<input type='radio' name='radiobtns' value='' />"; } });
            }
            grid=$("#grid").createLigerGrid({
                ...
                param: { 'flag': 1},
                keyID: "GOODS_ID",
                ajaxURL: "${ctx}/Base/Material/getGoodCategory.ajax",
                width:650
            });
...
<body>
    <form method="post" action="${ctx}/Base/Material/getGoodCategory.ajax" id="form1">
         <table>
            <tr>
                <td>作物：
                </td>
