	...
	<form method="post"
		...
		<div class="edit_panel">
			<table class="edit_table" width="99%" border="0" cellpadding="0"
				...
				<tr>
					<th width="80">
						<div class="th_text">地块编号:</div></th>
					<td><input name="txt_Plot_No" type="text"
						value=" ${bean.plotNo}" id="txt_Plot_No" disabled="disabled"
						class="aspNetDisabled inw" /></td>
				...
				<tr>
					...
					<td><input name="txt_CoFarmerName" type="text"
						value="${coFarmerBean.cofarmerName}" readonly="readonly"
					...
					<td><input name="txt_FarmerName" type="text"
						value="${farmerBean.farmerName}" readonly="readonly"
				...
				<tr>
					...
					<td colspan="3"><input name="txt_AreasDetials" type="text" tyle="width:500px;"
						value="${bean.areaDet}" id="txt_AreasDetials" class="inw" /></td>
				...
				<tr>
					...
					<td>
						<div style="float: left;">
							<input name="txt_Areas" type="text" value="${bean.areas}"
								id="txt_Areas" />
						</div>
					...
					<td><input name="txt_PlantBase" type="text"
						value="${plantBaseBean.plantbasesName}" id="txt_PlantBase"
				...
				<tr>
					...
					<td><select name="Drop_Facilities" id="Drop_Facilities">
							<option value="1"
								<c:if test="${'1' eq bean.facilities}">selected</c:if>>露地</option>
							<option value="2"
								<c:if test="${'2' eq bean.facilities}">selected</c:if>>塑料大棚</option>
							<option value="3"
								<c:if test="${'3' eq bean.facilities}">selected</c:if>>日光温室</option>
							<option value="4"
								<c:if test="${'4' eq bean.facilities}">selected</c:if>>地膜覆盖</option>
							<option value="5"
								<c:if test="${'5' eq bean.facilities}">selected</c:if>>其他设施</option>
					...
					<td><input name="txt_Notes" type="text" id="txt_Notes"
						class="inw" value="${bean.notes}" /></td>
		    ...
		    <input type="hidden" name="Plot_ID" id="Plot_ID" value="${bean.plotId}" /> 
			<input type="hidden" name="PlantBasesId" id="PlantBasesId" value="${plantBaseBean.plantbasesUid}" />
			<input type="hidden" name="CoFarmer_ID" id="CoFarmer_ID" value="${coFarmerBean.cofarmerId}" /> 
			<input type="hidden" name="Farmer_ID" id="Farmer_ID" value="${farmerBean.farmerId}" /> 
			<input type="hidden" name="areaId" id="areaId" value="${bean.areaId}" /> 
			 
			  <input type="hidden" id="bean.provinceCode" name="provinceValue" value="${bean.provinceAid}" /> 
			<input type="hidden" id="bean.cityCode" name="cityValue" value="${bean.cityAid}" /> 
			<input type="hidden" id="bean.districtCode" name="countyValue" value="${bean.countyAid}" /> 
			<input type="hidden" id="bean.streetCode" name="townValue" value="${bean.townAid}" /> 
